...
public class PluginActivityControl implements PluginActivityCallback {
	 ...
	 *            插件Activity
	 */
	public PluginActivityControl(String dexPath, Activity proxy, Activity plugin) {
		this(dexPath, proxy, plugin, null);
	 ...
	 *            分派给插件的Application
	 */
	public PluginActivityControl(String dexPath, Activity proxy, Activity plugin, Application app) {
		this.proxy = proxy;
		this.plugin = plugin;
		this.app = app;
		...
		try {
			...
			if(instrumentation==null){
                if (BuildConfig.DEBUG)
                    Log.e(LPluginConfig.TAG, "instrumentation==null");
			}
			pluginRef.call(
					// 方法名
					"attach",
					...
					new LPluginInstrument(instrumentation, dexPath),
					// IBinder token
					proxyRef.get("mToken"),
					// int ident
